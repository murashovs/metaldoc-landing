...
HTTP сейчас работает так:

```text
visitor -> 45.10.245.231:80 -> HAProxy http_front80
        -> backend robotpsa_web80 -> 127.0.0.1:18080
        -> robotpsa-site.service -> /srv/robotpsa/current
```

Плановая схема HTTPS после выпуска сертификата:
...
На момент подготовки, 10 июня 2026, домен был зарегистрирован, но зона `.ru` еще не отдавала делегацию и A-записи. Поэтому Let’s Encrypt пока не мог выпустить сертификат.
...
HAProxy принимает внешний HTTP и маршрутизирует по Host:
...
```bash
curl -fsSI --resolve robotpsa.ru:80:45.10.245.231 http://robotpsa.ru/
curl -fsS --resolve robotpsa.ru:80:45.10.245.231 http://robotpsa.ru/ | grep 'Робот ПСА'
```
...
Пока HTTPS не включен, fallback `_next` указывает на:

```text
http://robotpsa.ru/?sent=1#demo
```

После включения HTTPS заменить на:
